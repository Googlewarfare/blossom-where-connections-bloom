...
const Matches = () => {
  ...
  const fetchMatches = useCallback(async () => {
    ...
    try {
      ...
      const {
        data: matchesData,
        error: matchesError
      } = await supabase.from("matches").select("*").or(`user1_id.eq.${user.id},user2_id.eq.${user.id}`).order("created_at", {
        ascending: false
      });
      if (matchesError) throw matchesError;

      // Fetch profile data for each match
      const matchProfiles = await Promise.all((matchesData || []).map(async match => {
        const otherUserId = match.user1_id === user.id ? match.user2_id : match.user1_id;
        const {
          data: profileData
        } = await supabase.from("profiles").select("id, full_name, age, bio, location, occupation, verified").eq("id", otherUserId).single();

        // Fetch primary photo
        const {
          data: photoData
        } = await supabase.from("profile_photos").select("photo_url").eq("user_id", otherUserId).order("is_primary", {
          ascending: false
        }).order("display_order", {
          ascending: true
        }).limit(1).maybeSingle();
  ...
  const handleStartConversation = async (matchProfile: MatchProfile) => {
    try {
      ...
      const {
        data: existingConv
      } = await supabase.from("conversations").select("id").eq("match_id", matchProfile.match_id).maybeSingle();
      if (existingConv) {
  ...
  return <div className="min-h-screen min-h-[100dvh] w-full max-w-full overflow-x-hidden safe-area-inset">
      ...
      <div className="w-full px-4 py-8 max-w-7xl mx-auto box-border">
        ...
        {/* Matches Grid */}
        {matches.length === 0 ? <div className="text-center py-20">
            ...
            {matches.map((match, index) => <motion.div key={match.match_id} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
                ...
                <Card className="overflow-hidden hover:shadow-soft transition-smooth border-2">
                  <CardContent className="p-0">
                    ...
                    <div className="relative h-80 overflow-hidden bg-muted">
                      {match.photo_url ? <OptimizedImage src={match.photo_url} alt={match.full_name || "Match"} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/20 to-secondary/20">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
                        ...
                        <div className="space-y-1">
                          {match.location && <div className="flex items-center gap-2 text-sm">
                              <MapPin className="w-4 h-4" />
                              <span>{match.location}</span>
                            </div>}
                          {match.occupation && <div className="flex items-center gap-2 text-sm">
                    ...
                    <div className="p-6 space-y-4">
                      {match.bio && <p className="text-sm text-muted-foreground line-clamp-2">
                          {match.bio}
                        </p>}

                      <Button onClick={() => handleStartConversation(match)} className="w-full" size="lg">
                        <MessageCircle className="w-4 h-4 mr-2" />
                        Start Conversation
                      </Button>
