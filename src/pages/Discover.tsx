...
const Discover = () => {
  ...
  const hasUnlimitedSuperLikes =
    ...
    subscriptionStatus.subscriptions?.some(
      (sub) => sub.product_id === UNLIMITED_SUPER_LIKES_PRODUCT_ID,
    );
  ...
  useEffect(() => {
    ...
    const loadUserLocationAndFetchProfiles = async () => {
      try {
        ...
        const profilesWithDetails = await Promise.all(
          (profilesData || []).map(async (profile) => {
            ...
            const { data: interestsData } = await supabase
              ...
              .limit(5);
            const interests =
              interestsData?.map((item: any) => item.interests.name) || [];
            return {
              ...profile,
        ...
        const filteredProfiles = profilesWithDetails.filter(
          (p) => p !== null,
        ) as Profile[];
  ...
  const handleSwipe = async (action: "like" | "pass") => {
    ...
    try {
      ...
      if (action === "like") {
        const userId1 =
          user!.id < currentProfile.id ? user!.id : currentProfile.id;
        const userId2 =
          user!.id < currentProfile.id ? currentProfile.id : user!.id;
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
              ...
              <AdvancedFilters
                onFiltersApplied={() => window.location.reload()}
              />
              <Button
                onClick={() =>
          ...
          {/* Map or Card View */}
          {viewMode === "map" ? (
            <div className="h-[700px] w-full rounded-lg overflow-hidden shadow-xl">
              <MatchesMap
                profiles={
                  profiles.filter(
                ...
                <div className="relative w-full max-w-md">
                  ...
                  <div className="flex justify-center items-center gap-4 mt-8">
                    <ReportDialog
                      ...
                      trigger={
                        <Button
                          size="icon"
                          variant="outline"
                          className="h-12 w-12 rounded-full border-2 border-destructive/30 hover:bg-destructive hover:text-destructive-foreground"
                    ...
                    <Button
                      ...
                      title={
                        hasUnlimitedSuperLikes
                          ? "Send Super Like (Unlimited)"
                          : "Send Super Like ($4.99)"
                      }
                    >
