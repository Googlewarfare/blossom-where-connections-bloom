...
export const usePremium = () => {
  ...
  const hasPremium = subscriptionStatus?.subscriptions?.some(
    (sub) => sub.product_id === PREMIUM_FEATURES.BLOSSOM_PREMIUM
  ) ?? false;

  const hasUnlimitedSuperLikes = subscriptionStatus?.subscriptions?.some(
    (sub) => sub.product_id === PREMIUM_FEATURES.UNLIMITED_SUPER_LIKES
  ) ?? false;

  const hasReadReceipts = subscriptionStatus?.subscriptions?.some(
